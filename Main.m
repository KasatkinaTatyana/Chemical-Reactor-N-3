...
for t=t_0:h_t:t_End
    ...
    %% --------- tau вычисляется по программной траектории-----------------
%     T=InvStateDiff(y_tau);
%     h_tau = h_t / s([y_tau(1) -y_tau(2) T]);      
    %% -----tau вычисляется по траектории, получаемой из интегрирования системы-------
    h_tau = h_t / s(x_r(i,:));
    %% Другое вычисление h_tau. Используется линейная аппроксимация x_r.
    %% Этот вариант дает более плохую точность.
%     h_tau=0;
%     for j=0:N_tau
%         CA_temp = x_r(i,1) + (x_r(i+1,1) - x_r(i,1))*j/N_tau;
%         CB_temp = x_r(i,2) + (x_r(i+1,2) - x_r(i,2))*j/N_tau;
%         T_temp = x_r(i,3) + (x_r(i+1,3) - x_r(i,3))*j/N_tau;
%         h_tau=h_tau + h_t/N_tau / s([CA_temp CB_temp T_temp]);
%     end
    %% --------------------------------------------------------------------    
    i=i+1;
    tau=tau+h_tau;
